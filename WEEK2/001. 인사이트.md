...
그래서 bootc 메인 서버에서 podman으로 CentOS bootc 이미지를 만들고 레지스트리로 push했다. 그리고 클라이언트에서 bootc upgrade하면 자동으로 내가 원하는 컨테이너 파일 설정으로 부팅이 된다고 생각했다. ( 잘못 접근했으며 전용 bootc iso로 최초 설치해야하는 것으로 이해함 )
